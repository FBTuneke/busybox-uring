...
SEC("iouring") //.s = .is_sleepable = true
int cat(struct io_uring_bpf_ctx *ctx)
{
	...
	struct io_uring_cqe cqe = {};
      ...
      if(!context)
      {
            bpf_io_uring_emit_cqe(ctx, DEFAULT_CQ_IDX, CONTEXT_ERROR, 22222, 0); //Aus Kernelmodus zurückkehren und printen
            return 0; 
      }
      ...
      if(context->cq == OPEN_CQ_IDX)
      {
            ...
            {     
                  ...
                  // bpf_io_uring_emit_cqe(ctx, DEFAULT_CQ_IDX, cqe.res, 555555, 0);
                  // return 0;
            }
      }
      ...
      else if(context->cq == READ_CQ_IDX)
      {
            ...
            if (ret == 0) // CQE da 
            {
                  ...
                  {
                        bpf_io_uring_emit_cqe(ctx, DEFAULT_CQ_IDX, READ_ERROR, 22222, 0);
                        return 0;
                  }
            }
      }
